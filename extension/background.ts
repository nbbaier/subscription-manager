...
let activeSessions: Map<number, TrackingSession> = new Map();
...
async function fetchDomainMappings(): Promise<void> {
  try {
    ...
    domainMappings = cached.domainMappings || {};
  }
}
...
chrome.windows.onFocusChanged.addListener(async (windowId) => {
  if (windowId === chrome.windows.WINDOW_ID_NONE) {
    ...
    for (const [tabId, session] of activeSessions) {
      if (session.isActive) {
        session.totalSeconds += Math.floor((Date.now() - session.lastActiveTime) / 1000);
...
async function syncPendingEvents(): Promise<{ synced: number; failed: number }> {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const synced = result.results.filter((r: { success: boolean }) => r.success).length;
      const failed = result.results.filter((r: { success: boolean }) => !r.success).length;

      // Clear synced events, keep failed ones
      pendingEvents = pendingEvents.filter((_, i) => !result.results[i]?.success);
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'REFRESH_MAPPINGS') {
    fetchDomainMappings().then(() => {
      sendResponse({ success: true, count: Object.keys(domainMappings).length });
    });
    return true;
  }
