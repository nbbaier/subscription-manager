...
// Fetch user profile
async function getUserProfile(accessToken: string): Promise<GitHubUser> {
...
async function fetchContributionStats(accessToken: string): Promise<{
	totalCommits: number;
	totalPRs: number;
	totalIssues: number;
	recentRepos: string[];
...
function groupEventsByDate(events: GitHubEvent[]): Map<string, GitHubEvent[]> {
...
function calculateDailyActivityScore(events: GitHubEvent[]): {
	commits: number;
	prs: number;
	issues: number;
	reviews: number;
...
async function syncUsage(): Promise<GitHubSyncResult> {
	try {
		...
		if (!subscriptionId) {
			return {
				...
				newUsageEvents: 0,
				error:
					"No GitHub subscription found. Please create a subscription for GitHub first.",
			};
		}
		...
		const newEvents = events.filter(
			(e) => new Date(e.created_at).getTime() > lastSyncAt,
		);

		// Group events by date and log usage
		const eventsByDate = groupEventsByDate(newEvents);
		let newUsageEventsCount = 0;
		let totalCommits = 0;
		let totalPRs = 0;

		for (const [date, dateEvents] of eventsByDate) {
			const activity = calculateDailyActivityScore(dateEvents);
			...
			const repos = [
				...new Set(
					dateEvents.map((e) => e.repo.name.split("/")[1] || e.repo.name),
				),
			];
		...
		return {
			...
			newUsageEvents: newUsageEventsCount,
		};
	} catch (error) {
...
async function getActivitySummary(accessToken: string): Promise<{
	recentEvents: number;
	commits: number;
	pullRequests: number;
	issues: number;
	...
	const stats = await fetchContributionStats(accessToken);
