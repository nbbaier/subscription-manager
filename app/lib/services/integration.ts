...
function getIntegration(serviceName: string): Integration | null {
...
function upsertIntegration(
	...
	if (existing) {
		...
		const stmt = db.prepare(
			`UPDATE integrations SET ${updates.join(", ")} WHERE service_name = ?`,
		);
		stmt.run(...(values as (string | number | null)[]));

		const result = getIntegration(serviceName);
		if (!result) throw new Error(`Failed to update integration ${serviceName}`);
		return result;
	} else {
		const id = nanoid();
		const stmt = db.prepare(`
      ...
      `);
		...
		const result = getIntegration(serviceName);
		if (!result) throw new Error(`Failed to create integration ${serviceName}`);
		return result;
	}
}

// Store OAuth tokens
function storeTokens(serviceName: string, tokens: OAuthTokens): Integration {
	const expiresAt = tokens.expires_in
		? Date.now() + tokens.expires_in * 1000
		: null;

	return upsertIntegration(serviceName, {
...
function disconnect(serviceName: string): boolean {
	const stmt = db.prepare(`
...
function linkToSubscription(serviceName: string, subscriptionId: string): void {
	const stmt = db.prepare(`
...
function getIntegrationsSummary(): {
	connected: number;
	disconnected: number;
	error: number;
	integrations: Array<{
...
function generateAuthUrl(
	...
	const params = new URLSearchParams({
		client_id:
			process.env[`${serviceName.toUpperCase()}_CLIENT_ID`] ??
			(() => {
				throw new Error(
					`${serviceName.toUpperCase()}_CLIENT_ID not configured`,
				);
			})(),
		response_type: "code",
...
async function exchangeCodeForTokens(
	...
	const clientId =
		process.env[`${serviceName.toUpperCase()}_CLIENT_ID`] ??
		(() => {
			throw new Error(`${serviceName.toUpperCase()}_CLIENT_ID not configured`);
		})();
	const clientSecret =
		process.env[`${serviceName.toUpperCase()}_CLIENT_SECRET`] ??
		(() => {
			throw new Error(
				`${serviceName.toUpperCase()}_CLIENT_SECRET not configured`,
			);
		})();
...
async function refreshAccessToken(
	...
	try {
		...
		if (!response.ok) {
			updateSyncStatus(serviceName, "expired", "Token refresh failed");
			return null;
		}
		...
		updateSyncStatus(serviceName, "error", `Refresh failed: ${error}`);
		return null;
	}
}
...
async function getValidAccessToken(
	...
	if (isTokenExpired(integration)) {
		const tokens = await refreshAccessToken(serviceName);
...
// Delete integration
function deleteIntegration(serviceName: string): boolean {
