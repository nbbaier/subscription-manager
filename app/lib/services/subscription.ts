...
export class SubscriptionService {
  static getAllSubscriptions(): Subscription[] {
    const query = db.query('SELECT * FROM subscriptions ORDER BY created_at DESC');
    return query.all() as Subscription[];
  }

  static getActiveSubscriptions(): Subscription[] {
    const query = db.query("SELECT * FROM subscriptions WHERE status = 'active' ORDER BY created_at DESC");
    return query.all() as Subscription[];
  }

  static getSubscriptionById(id: string): Subscription | null {
    const query = db.query('SELECT * FROM subscriptions WHERE id = ?');
    return query.get(id) as Subscription | null;
  }

  static createSubscription(data: NewSubscription): Subscription {
    const id = nanoid();
    const now = Date.now();
  ...
  static updateSubscription(id: string, data: Partial<NewSubscription>): Subscription | null {
    const existing = this.getSubscriptionById(id);
  ...
  static deleteSubscription(id: string): boolean {
    ...
    const deleteEvents = db.prepare('DELETE FROM usage_events WHERE subscription_id = ?');
    deleteEvents.run(id);
